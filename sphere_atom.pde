...
void draw(){
  ...
  text("Number of Bonds: "+test.numOfBonds,(0.95)*width,0.1*height,10);
  text("Number of Electron Domains: "+test.possibleBonds,(0.95)*width,0.13*height,10);
  text("Electronic Geometry: "+example.electronic(test.possibleBonds),(0.95)*width,0.16*height,10);
  text("Molecular Geometry: "+example.molecular(test.possibleBonds,test.numOfBonds),(0.95)*width,0.19*height,10);
  text("ex. "+example.ex(test.possibleBonds,test.numOfBonds),(0.95)*width,0.22*height,10);
  popMatrix();
...
void numPress(int n){
  if(creatingProcess==1){
        ...
        newPosBonded=n;
        if(newBonded>newPosBonded){ int temp= newBonded;  newBonded=newPosBonded;  newPosBonded=temp;}
...
void keyPressed(){
   ...
   if(key=='r'||key=='R'){
     rx=0;ry=0;rz=0; test.distance=(int)100;   rotationList1.clear(); rotationList2.clear(); test.update();
   }
}
void mouseWheel(MouseEvent event) {
  ...
  if(rchoose==3){
    if(test.distance>40 && e>0){
    test.distance-=(5*e);} else if(e<0){test.distance-=(5*e);}   
    test.update();}
  if(rchoose==4){
    if(test.radius>4 && (e>0)){
    test.radius-=e;} else if(e<0){test.radius-=e;}   
    test.update();}
}